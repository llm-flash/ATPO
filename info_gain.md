...
Currently the advantage of a node is computed as V' + beta * H * C', where V' is the normalized value, beta is an annealed learning rate (starting at 0.8 and decaying to 0.3), H is the entropy, and C' is the normalized curiosity. The raw curiosity is defined as V - P(ans) where V is the unnormalized value and P(ans) is the ground-truth probability at that node (i.e. the probability assigned to the ground-truth answer conditioned on all the context up to and including that node).
