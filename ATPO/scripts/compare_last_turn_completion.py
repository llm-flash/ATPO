"""Generate direct and prompt-modified answers with vLLMRolloutWithTools.

This script intentionally runs the rollout in validation/greedy mode and with no
configured tool instances. That exercises vLLMRolloutWithTools.generate_sequences
while preventing actual tool execution.

For a quick test, you can use the --num_prompts argument to avoid using all the
prompts in the input file.

To get the data for this script, you need to use convert_prompt_format.py first.
That will give you the file to use for --prompts-file. The final results that this
script prints will tell you the accuracy with full and reduced context, and the 
agreement in final answer between the two. It will also tell you the average time
required for the calls to vLLMRolloutWithTools.generate_sequences for each case.
Note that the calls are batched, so the times it gives you are per-batch, not 
per-sample. The batch size is controlled by --batch-size.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompts-file", help=".jsonl/.json/.txt prompt file.")
    ...
    parser.add_argument("--prompt-length", type=int, default=6192) # Because we are not starting from the original prompt, but rather a truncated version of the full response
    parser.add_argument("--response-length", type=int, default=6192)
...
def build_rollout_config(args: argparse.Namespace):
    # IMPORTANT: Note that we will be in validation mode by default because the meta_info of the DataProto will have validate=True.
    samples_per_tree = 1
...
def render_prompt(record: dict[str, Any], tokenizer) -> str:
    ...
    if "input" in record:
        return record["input"]
    raise ValueError(f"Record lacks messages, prompt, or input: {record}")
...
def build_dataproto(records: list[dict[str, Any]], tokenizer, max_prompt_length: int) -> DataProto:
    ...
    for record in records:
        prompt = render_prompt(record, tokenizer)
...
def decode_response(output: DataProto, tokenizer, index: int) -> str:
    prompt_length = output.batch["prompts"].shape[-1]
    valid_response_length = output.batch["attention_mask"][index, prompt_length:].sum().item()
    response_ids = output.batch["responses"][index, :valid_response_length]
...
def generate_batch(
    ...
    for i, record in enumerate(records):
        response = decode_response(output, tokenizer, i)
        raw_answer = extract_solution(response)
        # Extract the content inside \boxed{} from the <answer> tag content
        boxed_answer = None
        if raw_answer is not None:
            boxed_str = last_boxed_only_string(raw_answer)
            if boxed_str is not None:
                boxed_answer = remove_boxed(boxed_str)
        results.append(
            {
                "id": i,
                "response": response,
                "answer": boxed_answer if boxed_answer is not None else raw_answer,
            }
        )
    return results
...
def main() -> None:
    ...
    with output_path.open("w") as f:
        for start in range(0, len(records), args.batch_size):
            ...
            direct = generate_batch(rollout, tokenizer, batch_records, args.prompt_length, timings=direct_timings)
            modified_records = [
                modify_prompt(record)
                for record in batch_records
            ]
            modified = generate_batch(rollout, tokenizer, modified_records, args.prompt_length, timings=modified_timings)
                    ...
                    json.dumps(
                        {
                            ...
                            "modified_prompt": modified_record,
...
def _print_statistics(output_path: Path, breakdown_by_id: bool = False) -> None:
    """Read the output JSONL and print agreement statistics."""
    direct_correct = 0
    modified_correct = 0
    both_correct = 0
    neither_correct = 0
    direct_only_correct = 0
    modified_only_correct = 0
    d_correct_and_changed = 0
    m_correct_and_changed = 0
    agree = 0
    disagree = 0
    total = 0
    total_changed = 0
    ...
    with output_path.open() as f:
        for line in f:
            ...
            gt = rec["original_prompt"].get("ground_truth", "").strip().lower()
            d_ans = (rec["direct"].get("answer") or "").strip().lower()
            m_ans = (rec["modified"].get("answer") or "").strip().lower()

            d_match = em_check(d_ans, gt) if gt and d_ans else False
            m_match = em_check(m_ans, gt) if gt and m_ans else False
            ...
            if rec["changed"]:
                total_changed += 1
                if d_match:
                    d_correct_and_changed += 1
                if m_match:
                    m_correct_and_changed += 1
                if d_ans == m_ans:
                    agree += 1
                else:
                    disagree += 1
                ...
                per_id_rows.append(
                    {
                        "id": sample_id,
                        "direct_answer": d_ans,
                        "modified_answer": m_ans,
                        "ground_truth": gt,
                        "direct_correct": d_match,
                        "modified_correct": m_match,
                        "agree": d_ans == m_ans,
                    }
                )
    ...
    print(f"\nDirect answer accuracy: {direct_correct}/{total} "
          f"({direct_correct / total * 100:.1f}%)")
    print(f"Modified answer accuracy: {modified_correct}/{total} "
          f"({modified_correct / total * 100:.1f}%)")
    print(f"\nBoth correct: {both_correct}/{total} ({both_correct / total * 100:.1f}%)")
    print(f"Only direct correct: {direct_only_correct}/{total} ({direct_only_correct / total * 100:.1f}%)")
    print(f"Only modified correct: {modified_only_correct}/{total} ({modified_only_correct / total * 100:.1f}%)")
    print(f"Neither correct: {neither_correct}/{total} ({neither_correct / total * 100:.1f}%)")
    if total_changed > 0:
        print("\n\nStatistics for instances where prompt actually changed (i.e., modified prompt differs from direct prompt):")
        print(f"Direct correct: {d_correct_and_changed}/{total_changed} ({d_correct_and_changed / total_changed * 100:.1f}%)")
        print(f"Modified correct: {m_correct_and_changed}/{total_changed} ({m_correct_and_changed / total_changed * 100:.1f}%)")
        print(f"\nDirect and modified agree: {agree}/{total_changed} ({agree / total_changed * 100:.1f}%)")
        print(f"Direct and modified disagree: {disagree}/{total_changed} ({disagree / total_changed * 100:.1f}%)")
    else:
        print("\nNo instances where prompt changed (i.e., modified prompt differs from direct prompt).")
    if breakdown_by_id:
        print("\nBY ID")
        print("=" * 60)
        for row in per_id_rows:
            print(
                f"id={row['id']}: direct={'correct' if row['direct_correct'] else 'wrong'}, "
                f"modified={'correct' if row['modified_correct'] else 'wrong'}, "
                f"agree={'yes' if row['agree'] else 'no'}"
            )
            print(f"  gt={row['ground_truth']}")
            print(f"  direct={row['direct_answer']}")
            print(f"  modified={row['modified_answer']}")
    print("=" * 60 + "\n")
