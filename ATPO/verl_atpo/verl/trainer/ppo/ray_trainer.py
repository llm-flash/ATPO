        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    ############################### Begin logging code ###############################
                    if gen_batch_output.meta_info and "curiosity_stats" in gen_batch_output.meta_info:
                        curiosity_stats = gen_batch_output.meta_info["curiosity_stats"]
                        for k, v in curiosity_stats.items():
                            logger.log(
                                data={f"curiosity/{k}": v},
                                step=self.global_steps,
                            )
                        logger.log(
                            data={f"curiosity/annealed_multiplier": gen_batch_output.meta_info.get("annealed_multiplier", 0.0)},
                            step=self.global_steps,
                        )
                    ############################### End logging code ###############################
