...
from verl.trainer.ppo.core_algos import (
    ...
    compute_policy_loss_gspo_soft_clip,
    compute_policy_loss_gspo_turn,
    compute_policy_loss_gspo_turn_soft_clip,
    compute_policy_loss_gppo,
    compute_policy_loss_gppo_half,
)
...
class DataParallelPPOActor(BasePPOActor):
    def __init__(
        ...
        print(
            f"Actor enable_entropy_balanced_clipping={self.enable_entropy_balanced_clipping}"
        )
        print(
            f"Actor enable_entropy_balanced_advantage={self.enable_entropy_balanced_advantage}"
        )
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.policy_loss=="gspo":
                    ...
                    elif self.config.policy_loss=="gppo":
                        print("========================Gppo policy loss============================")
                        pg_loss, pg_metrics = compute_policy_loss_gppo(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
                            entropy=entropy,
                            advantages=advantages,
                    ...
                    elif self.config.policy_loss=="gppo_half":
                        print("========================Gppo half policy loss============================")
                        pg_loss, pg_metrics = compute_policy_loss_gppo_half(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
                            entropy=entropy,
                            advantages=advantages,
                    ...
                    elif self.config.policy_loss=="grpo":
                        print("========================Entropy balanced policy loss============================")
                        pg_loss, pg_metrics = (
                            compute_policy_loss_entropy_balanced_clipping(
                                old_log_prob=old_log_prob,
                                log_prob=log_prob,
                                ...
                                advantages=advantages,
                                response_mask=response_mask,
                                enable_entropy_balanced_clipping=self.enable_entropy_balanced_clipping,
                                enable_entropy_balanced_advantage=self.enable_entropy_balanced_advantage,
                                cliprange=clip_ratio,
                                cliprange_low=clip_ratio_low,
                                cliprange_high=clip_ratio_high,
                                clip_ratio_c=clip_ratio_c,
                        ...
                        raise NotImplementedError(
                            f"Policy loss {self.config.policy_loss} not implemented"
                        )
                    ...
                    data = pg_metrics
                    data["actor/pg_loss"] = pg_loss.detach().item()
