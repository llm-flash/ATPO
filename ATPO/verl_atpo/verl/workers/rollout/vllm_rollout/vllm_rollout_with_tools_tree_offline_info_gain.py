...
class ToolTreeNode:
    ...
    def __init__(
        ...
        # Info gain tracking
        self.gt_prob = None  # Ground truth answer probability for info gain calculation
        self.pseudo_response_info = None # All nodes in a tree use the same pseudo response since the ground truth answer is the same; thus this gets inherited from the parent node
    ...
    def add_child(self, child_node: 'ToolTreeNode'):
        ...
        # This is a hacky way to guarantee every node inherits the pseudo response info from the root node
        child_node.pseudo_response_info = self.pseudo_response_info
...
class vLLMRolloutWithTools(vLLMRollout):
    ...
    def _build_initial_trees(self, prompt_token_ids_list: List[List[int]], 
                            initial_rollouts_list: List[int],
                            pseudo_response_info = None
                            ) -> List[ToolTreeNode]:
        ...
        for i, prompt_ids in enumerate(prompt_token_ids_list):
            ...
            if pseudo_response_info is not None:
                root_node.pseudo_response_info = pseudo_response_info[i]
    ...
    def get_ground_truth_probs(self, contexts, pseudo_responses, gt_token_indices):
        ...
        pseudo_log_probs = self.inference_engine.generate(
            prompt_token_ids=pseudo_rollouts,
            sampling_params=sampling_params,
            use_tqdm=False,
        )
        # The format of pseudo_log_probs[1].prompt_logprobs is a list of dicts, where each dict maps the token_id 
        # to a Logprob object with the logprob, the rank, and the decoded token
        # Since we know the ground-truth token indices, we can extract the corresponding logprobs
        # We can also double-check that the decoded tokens match the ground-truth tokens for sanity
        ...
        for i in range(len(pseudo_rollouts)):
            ...
            gt_token_indices[i] = [indx + len(contexts[i]) for indx in gt_token_indices[i]]
            for gt_token_idx in gt_token_indices[i]:
                logprob_info = pseudo_log_probs[i].prompt_logprobs[gt_token_idx]
                gt_token_id = pseudo_rollouts[i][gt_token_idx]
                if gt_token_id in logprob_info:
                    logprob = logprob_info[gt_token_id].logprob
                    decoded_token = logprob_info[gt_token_id].decoded_token
                    # NOTE: Need to use convert_tokens_to_string to account for the fact that leading spaces are represented by Ġ in the output from vllm
                    assert self.tokenizer.convert_tokens_to_string([decoded_token]) == self.tokenizer.decode([gt_token_id]), f"Decoded token does not match expected for gt token index {gt_token_idx} in sample {i}"
                    log_probs.append(logprob)
                else:
                    raise ValueError(f"GT token ID {gt_token_id} not found in logprob info for gt token index {gt_token_idx} in sample {i}")
            log_probs_sum = sum(log_probs) # Sum logprobs to get total log prob for the ground-truth answer
            gt_probs.append(np.exp(log_probs_sum).item())
    ...
    def _generate_chain_for_nodes(self, initial_nodes: List[ToolTreeNode],
                                 ...
                                 phase_name: str = "",
                                 calc_gt_probs: bool = False,
                                 calc_initial_node_gt_probs: bool = True):
        ...
        # Get ground-truth logprobs for answers directly after the initial prompts (i.e. before any generation)
        if calc_gt_probs and calc_initial_node_gt_probs:
            # NOTE: outputs[i] corresponds to current_nodes[i]
            full_seq_no_pseudo_resp = [node.curr_token_ids for node in current_nodes]
            pseudo_resps_with_gt = [node.pseudo_response_info[0] for node in current_nodes]
        ...
        while current_nodes:
            ...
            if calc_gt_probs:
                # The child nodes exhibit the desired behavior (i.e. ending on a tool result)
                tool_result_node_indices = [i for i, node in enumerate(child_nodes) if ("_tool" in node.node_uid and node.result_mask and node.result_mask[-1] == 0)]
                if len(tool_result_node_indices) > 0:
                    full_seq_no_pseudo_resp = [child_nodes[i].curr_token_ids for i in tool_result_node_indices]
                    pseudo_responses = [child_nodes[i].pseudo_response_info[0] for i in tool_result_node_indices]
    ...
    # Create the a pseudo-response for each prompt which uses the corresponding ground-truth answer
    def get_pseudo_responses(self, prompts):
        batch_dict = prompts.non_tensor_batch
        ...
        task_type = "search" # hard-coded for now since we are only working with search tasks
        ...
        # len_st = len(self.tokenizer("\n<think>\nNow there's enough information to answer.</think>\n<answer>\nThe final answer is \\boxed{")['input_ids'][0])
        pseudo_response_template = "\n<think>\nNow there's enough information to answer.</think>\n<answer>\nThe final answer is \\[ \\boxed{{{ground_truth}}} \\] </answer>"
        len_st = len(self.tokenizer("\n<think>\nNow there's enough information to answer.</think>\n<answer>\nThe final answer is \\[ \\boxed{")['input_ids'])
        len_ed = len(self.tokenizer("} \\] </answer>")['input_ids'])
        # For Qwen2.5, '{' has ID 90 and '}' has ID 92
        surrounding_chars = ['{', '}']
        expected_start_token_id = self.tokenizer(surrounding_chars[0])['input_ids'][0]
        expected_end_token_id = self.tokenizer(surrounding_chars[1])['input_ids'][0]
        pseudo_resps_with_gt = [self.tokenizer(pseudo_response_template.format(ground_truth=ground_truth))['input_ids'] for ground_truth in ground_truths]
        gt_token_indices = [] # Stores the indices corresponding to ground-truth tokens for each sample in the batch

        # In certain cases, the a ground-truth answer token may merge with adjacent tokens. We can tell 
        ...
        for i, resp_with_gt in enumerate(pseudo_resps_with_gt):
            tokenized_gt_with_markers = self.tokenizer(f"{surrounding_chars[0]}{ground_truths[i]}{surrounding_chars[1]}")['input_ids']
            true_len_st = len_st
            len_gt = len(tokenized_gt_with_markers)
            ...
            if not merge_detected:
                assert len_gt == len(resp_with_gt) - len_st - len_ed, "Length mismatch even though no merges detected."
            gt_token_indices.append(list(range(true_len_st, true_len_st + len_gt)))
            decoded_ground_truth = self.tokenizer.decode(resp_with_gt[slice(gt_token_indices[i][0], gt_token_indices[i][-1] + 1, 1)])
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if is_validate:
            ...
            pseudo_response_info = self.get_pseudo_responses(prompts)
        ...
        with self.update_sampling_params(**kwargs):
            ...
            root_nodes = self._build_initial_trees(prompt_token_ids_list, initial_rollouts_list, pseudo_response_info=pseudo_response_info)
            
            # Compute the ground-truth probabilities when putting the pseudo-responses directly after the prompt
            # For some reason, the initial branches from these roots have the exact same tokens as the roots,
            # so the ground-truth probabilities are the same for them. Thus, no need to repeat the computation.
            if not is_validate:
                full_seq_no_pseudo_resp = [node.curr_token_ids for node in root_nodes]
                pseudo_resps_with_gt = [node.pseudo_response_info[0] for node in root_nodes]
            ...
            # Assign the previously computed ground-truth probabilities to the initial branches since they have the same 
            # tokens
            if not is_validate:
                for branch in initial_branches:
                    branch.gt_prob = branch.parent_node.gt_prob
            ...
            self._generate_chain_for_nodes(
                ...
                phase_name="Phase 2",
                calc_gt_probs=not is_validate,  # Only calculate GT probabilities if not in validation mode
                calc_initial_node_gt_probs=False,  # Ground truth probs for the initial nodes were already computed above
            )
